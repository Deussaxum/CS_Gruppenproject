...
def main():
    ...
    for index, question in enumerate(personality_questions):
        slider_label = st.markdown(question)
        slider = st.slider(f"Rate from 1 (Strongly Disagree) to 5 (Strongly Agree)##personality_{index}", 1, 5, 3)
    ...
    for index, question in enumerate(skill_questions):
        slider_label = st.markdown(question)
        slider = st.slider(f"Rate from 1 (Strongly Disagree) to 5 (Strongly Agree)##skill_{index}", 1, 5, 3)
    ...
    if st.button("Submit"):
        ...
        total_fit_percentages = calculate_total_fit(list(personality_scores.values()), skill_scores, industry_scores)

        # Display Radar Charts and Fit Percentages
        plot_radar_chart(list(personality_scores.keys()), list(personality_scores.values()), 'Your Big Five Personality Traits')
        ...
        st.write(f"Recommended Industry: {recommended_industry}")
